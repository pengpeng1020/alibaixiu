...
const validateUser = user => {
	...
	const schema = {
		...
		status: Joi.number().valid(0, 1).error(new Error('status不符合规则'))
		// role: Joi.string().valid('normal', 'admin').error(new Error('role不符合规则'))
	};
	// 验证
...
User.findOne({'email': 'pengpeng@njust.edu.cn'}).then(async result => {
	if (result == null) {
		// 生成盐
